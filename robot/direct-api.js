  ...
  const apiClient = {
    ...
    async createTask(taskInfo) {
      try {
        ...
        // First try the supported 'chassis/moves' endpoint that's available according to status
        console.log(`Creating task using moves endpoint for robot ${serialNumber}`);
        try {
          // Using the available 'chassis/moves' endpoint from status API response
          const moveResponse = await axiosInstance.post('/chassis/moves', {
            ...taskData,
            moveType: taskData.taskType || 2, // Map task type to move type
            movePoints: taskData.taskPts || []
          });
          
          if (moveResponse.data && moveResponse.data.move_id) {
            console.log(`Task created successfully via moves API for robot ${serialNumber}, ID: ${moveResponse.data.move_id}`);
            return moveResponse.data.move_id;
          }
        } catch (moveError) {
          console.log(`Moves endpoint failed, falling back to task API: ${moveError.message}`);
        }
        
        // Fall back to the 'task/create' endpoint without version number
        try {
          ...
          if (response.data && response.data.data && response.data.data.taskId) {
            ...
            throw new Error('Unexpected response format from task creation');
          }
        } catch (taskError) {
