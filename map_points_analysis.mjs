...
try {
  ...
  console.log('\nNAMED POINTS:');
  data.points
    .filter(p => p.name && !p.name.startsWith('6823a'))
    .forEach(p => {
  ...
  // Identify point pairs (rack and docking pairs)
  console.log('\nRACK-DOCKING PAIRS:');
  const rackNames = rackPoints.map(r => r.name.replace('_load', ''));
  rackNames.forEach(baseName => {
    const rack = data.points.find(p => p.name === `${baseName}_load`);
    const docking = data.points.find(p => p.name === `${baseName}_load_docking`);
    
    if (rack && docking) {
      console.log(`- ${baseName}:`);
      console.log(`  - Rack: [${rack.x.toFixed(2)}, ${rack.y.toFixed(2)}], yaw: ${rack.yaw}`);
      console.log(`  - Dock: [${docking.x.toFixed(2)}, ${docking.y.toFixed(2)}], yaw: ${docking.yaw}`);
      
      // Calculate distance between rack and docking
      const distance = Math.sqrt(
        Math.pow(rack.x - docking.x, 2) + 
        Math.pow(rack.y - docking.y, 2)
      );
